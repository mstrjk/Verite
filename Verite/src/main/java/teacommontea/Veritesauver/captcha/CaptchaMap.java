...
public final class CaptchaMap implements Listener {
    ...
    public CaptchaMap(JavaPlugin plugin, Messages messages, CaptchaManager manager) {
        ...
        teacommontea.util.chat.ChatRouter.register(EventPriority.LOWEST, true, event -> {
    ...
    private static final class Session {
        ...
        BufferedImage image() {
            ...
            for (int i = 0; i < 25; i++) {
                g.fillRect(Math.abs(code.hashCode() + i * 31) % 128,
    ...
    private void startTick(Player player, Session s) {
        UUID u = player.getUniqueId();
        s.task = teacommontea.util.sched.Sched.executeForRepeating(player, () -> {
